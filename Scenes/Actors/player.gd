...
func _update_animation(_direction: float) -> void:
	if is_dead:
		...
		_play_anim_safe("idle") # Matches lowercase "idle" from screenshot
...
func die() -> void:
	...
	# --- STOP LEVEL MUSIC TO PREVENT OVERLAPPING ---
	var level_music = get_tree().current_scene.get_node_or_null("MusicPlayer")
	if level_music and level_music is AudioStreamPlayer:
		level_music.stop()
	
	# Also check if it's inside your AudioManager singleton just in case
	if AudioManager and AudioManager.has_node("MusicPlayer"):
	...
	# Matches "Dead" from your screenshot
	_play_anim_safe("Dead")
